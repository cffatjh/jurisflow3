...
import { Clock, Briefcase, Scale, CreditCard, CheckSquare, Calendar, ChevronRight } from './Icons';
...
const Dashboard: React.FC = () => {
  ...
  const todayTasksDue = tasks.filter(tsk => {
    if (tsk.status === 'Done') return false;
    ...
    <div className="p-8 overflow-y-auto h-full space-y-8 bg-gray-50/50">
        ...
        <StatCard
          ...
          subtext={
            overdueInvoices.length > 0
              ? <span className="text-red-600 font-semibold">{overdueInvoices.length} Overdue Invoices</span>
              : <span className="text-amber-600 font-semibold">{tasks.filter(t => t.priority === 'High' && t.status !== 'Done').length} High Priority</span>
          }
          icon={CheckSquare}
      ...
      <div className="bg-white rounded-xl shadow-card border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Avukat/Associate/Partner common */}
          <button
          ...
          {/* Admin/Partner extra emphasis: finance, otherwise docs */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-card border border-gray-100">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-bold text-slate-800">{t('high_priority_tasks')}</h3>
            </div>
            <div className="space-y-3">
              {tasks.length === 0 && <div className="text-gray-400 text-sm">No pending tasks.</div>}
              {tasks.filter(t => t.status !== 'Done').slice(0, 4).map(task => (
                <div key={task.id} className="flex items-center p-3 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors group">
                  <div className={`w-2 h-2 rounded-full mr-4 ${task.priority === 'High' ? 'bg-red-500' : 'bg-blue-500'}`}></div>
        ...
        {/* Right Column: Calendar & Recent Matters */}
