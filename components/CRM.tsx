...
const CRM: React.FC = () => {
  ...
  const performConflictCheck = (e: React.FormEvent) => {
      ...
      // Search across Clients and Leads
      const matches = [
          ...clients.filter(c => c.name.toLowerCase().includes(q)).map(c => ({...c, type: 'Client'})),
                     ...
                     {clients.map(client => (
                        <tr
                          ...
                          className={`transition-colors ${
                            highlightClientId === client.id ? 'bg-indigo-50' : 'hover:bg-gray-50'
                          }`}
                        >
